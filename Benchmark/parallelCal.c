...
int main(int argc, char **argv) {
	...
	// initialize rthread_t
	thread = (rthread_t*)malloc(thread_num*sizeof(rthread_t));
	...
	for (i = 0; i < thread_num; ++i)
		rthread_create(&thread[i], NULL, &parallel_calculate, &counter[i]);

	//rthread_schedule(0);

	for (i = 0; i < thread_num; ++i)
		rthread_join(thread[i], NULL);
